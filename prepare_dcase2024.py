...
def get_test_data_list(test_dirs, dataset_name=None):
    root_path = "./"
    test_file_list = []
    test_label_list = []
    test_machine_list = []
    ...
    for td in test_dirs:
        ...
        gt_list = np.array(pd.read_csv(
            root_path + dataset_name+'_task2_evaluator/ground_truth_data/ground_truth_' + machine + '_section_00_test.csv',
            header=None).iloc[:, 1]).tolist()
        s_list = np.array(pd.read_csv(
            root_path + dataset_name+'_task2_evaluator/ground_truth_domain/ground_truth_' + machine + '_section_00_test.csv',
            header=None).iloc[:, 1] == 0).tolist()
...
def get_dcase2024(train_split="train+add", dataset_name=None):
    """ Args:
    ...
    if train_split == "train+add":
        train_dirs = sorted(data_config[dataset_name + "_train_dirs"]) + sorted(data_config[dataset_name + "_add_dirs"])
        train_file_list, train_label_list, train_source_list, train_machine_names, train_file_attrs = prepare_data(
            train_dirs)
    elif train_split == "train":
        train_dirs = data_config[dataset_name + "_train_dirs"]
        train_file_list, train_label_list, train_source_list, train_machine_names, train_file_attrs = prepare_data(
            train_dirs)
    elif train_split == "add":
        train_dirs = data_config[dataset_name + "_add_dirs"]
        train_file_list, train_label_list, train_source_list, train_machine_names, train_file_attrs = prepare_data(
            train_dirs)
    valid_dirs = sorted(data_config[dataset_name + "_valid_dirs"])
    test_dirs = sorted(data_config[dataset_name + "_test_dirs"])
    valid_file_list, valid_label_list, valid_source_list, valid_machine_names, valid_file_attrs = prepare_data(
        valid_dirs)
    test_file_list, test_label_list, test_source_list, test_machine_names, test_file_attrs = get_test_data_list(
        test_dirs, dataset_name)
